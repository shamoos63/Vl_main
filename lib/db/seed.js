const { getDb, properties, propertyTranslations, blogs, blogTranslations, admins } = require('./index');
  ...
  try {
    ...
    try {
      ...
      if (errorStr.includes('UNIQUE constraint failed') || errorStr.includes('SQLITE_CONSTRAINT')) {
        ...
        adminUser = await db.select().from(admins).where(eq(admins.email, 'victoria.lancaster@selectproperty.ae')).limit(1);
